...
const stats = [
  { label: "Questions Answered", value: "32" },
  { label: "Topics Covered", value: "6" },
  { label: "Open", value: "9am–5pm · 7 Days" },
];

export default function AboutFaqHeroSection() {
  const sectionRef = useRef<HTMLElement>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(
        sectionRef.current?.querySelectorAll(".faq-hero-anim") ?? null,
        { opacity: 0, y: 24 },
        { opacity: 1, y: 0, stagger: 0.12, duration: 0.9, ease: "power3.out", delay: 0.2 }
      );
      gsap.fromTo(
        sectionRef.current?.querySelectorAll(".faq-stat-anim") ?? null,
        { opacity: 0, y: 30 },
  ...
  return (
    ...
    >
      ...
      <img
        ...
        alt="Club Nautical yacht in the Whitsundays"
      ...
      <div style={{ position: "relative", zIndex: 2, textAlign: "center", padding: "60px 24px 0" }}>
        <div className="faq-hero-anim" style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22, justifyContent: "center" }}>
        ...
        >
          Everything you need to know about chartering a yacht with Club Nautical, in one place.
        </p>
      </div>

      <div
        className="faq-stat-anim"
        style={{
          position: "absolute",
          left: "50%",
          bottom: 0,
      ...
      >
        {stats.map((s, i) => (
          ...
          >
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(18px,2.2vw,26px)", color: "#003052", lineHeight: 1 }}>
              {s.value}
            </div>
